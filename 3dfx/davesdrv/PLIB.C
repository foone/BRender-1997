...
static struct br_tv_template_entry primitiveLibraryTemplateEntries[] = {
	...
	{BRT(PARTS_TL),			&PrimPartsTokens,	BRTV_QUERY | BRTV_ALL | BRTV_ABS,	BRTV_CONV_LIST },
...
{
	void PrimitiveStateClearTemplates(void);
	void BufferStoredClearTemplate(void);
	extern float fog_constant;

	CLEAR_TEMPLATE(primitiveLibrary);
	PrimitiveStateClearTemplates();
	BufferStoredClearTemplate();

	/* Constant needed for computing fogging table */
	fog_constant = 4.0f / log(2.0f);
