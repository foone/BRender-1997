...
static void updateRanges(struct br_primitive_state *self)
{
	...
	self->cache.comp_offsets[C_SX] =  BrIntToScalar((self->out.colour->pm_width  / 2 + self->out.colour->pm_base_x) * 16);
	self->cache.comp_scales[C_SX]  =  BrIntToScalar((self->out.colour->pm_width  / 2) * 16);
	self->cache.comp_offsets[C_SY] =  BrIntToScalar((self->out.colour->pm_height / 2 + self->out.colour->pm_base_y) * 16);
	self->cache.comp_scales[C_SY]  = -BrIntToScalar((self->out.colour->pm_height / 2) * 16);
...
br_boolean texture_cache_created = BR_FALSE;
...
#if 1
		/* Temporary hack, as the 3DFX won't allow textures to span 2M boundaries */
		if (size > 1024*2048)
			size = 1024*2048;

		if (!TextureCacheInit(base, size, FXTextureFreedCallback))
			return(BRE_DEV_NO_MEMORY);
#else
		if (size > 1024*2048) {
			if (!TextureCacheInit(base, 1024*2048, FXTextureFreedCallback))
				return(BRE_DEV_NO_MEMORY);
			size -= 1024*2048;
			base += 1024*2048;
			while (size > 1024*2048) {
				/* Note a small gap is left to prevent blocks being concatenated */
				if (!TextureCacheAdd(base+16, 1024*2048-16))
					return(BRE_DEV_NO_MEMORY);
				size -= 1024*2048;
				base += 1024*2048;
			}
			if (size > 16)
				if (!TextureCacheAdd(base+16, size-16))
					return(BRE_DEV_NO_MEMORY);
		} else {
			if (!TextureCacheInit(base, size, FXTextureFreedCallback))
				return(BRE_DEV_NO_MEMORY);
...
//			alpha_value = self->prim.alpha_val;
		...
		br_token prim_type)
{
	...
	/* Update the ranges - since the texture or block type is likely to change with
	 * every block anyway unless very large numbers of non-textured materials are used.
	 * This fixes several bugs with subpixelmaps. */
	updateRanges(self);
	...
	*block_changed = BR_TRUE;
	...
	if (self->out.colour->buffer_type != BT_BACKSCREEN) {
	...
	*rpb = &pb->p;
	...
	Set3DfxState(self, pb->flags_cmp);
