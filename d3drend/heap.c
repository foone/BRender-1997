...
void BR_ASM_CALL OpHeapAddTriangle(struct brp_block *block, brp_vertex *v0, brp_vertex *v1, brp_vertex *v2)
{
	...
	p->v[1] = heapVertexAdd(rend.renderer->state.hidden.heap, v1,!(rend.block->flags & BR_PRIMF_CONST_DUPLICATE));
	p->v[2] = heapVertexAdd(rend.renderer->state.hidden.heap, v2,!(rend.block->flags & BR_PRIMF_CONST_DUPLICATE));
