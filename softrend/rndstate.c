 ...
 *
 * $Id: rndstate.c 2.19 1997/02/17 12:28:57 jon Exp $
 * $Locker: $
 *
...
BR_RCS_ID("$Id: rndstate.c 2.19 1997/02/17 12:28:57 jon Exp $");
		...
		(struct br_renderer *self, struct br_renderer_state_stored *save, br_uint_32 mask)
{
	...
	/*
	 * Kinda gross hack for <REDACTEDGAME>.  We don't actually copy primitive state, we just copy
	 * pointers where possible, so we must ensure that the default primitive state is used
	 * when a material's state is no longer being used
	 */
	br_error r = StateCopyToStored(save, &self->state, mask, save);
	self->state.pstate = self->default_primitive_state;
	return r;
#else
		...
		(struct br_renderer *self, br_uint_32 mask)
{
	 ...
	 */
	 ...
	 /*
	  * Kinda gross hack for <REDACTEDGAME>.  We don't actually copy primitive state, we just copy
	  * pointers where possible, so we must use the default primitive state (and reset it)
	  */
	PrimitiveStateStateDefault(self->default_primitive_state, mask);

	self->default_state->pstate = self->default_primitive_state;
#else
