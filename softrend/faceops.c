 ...
 *
 * $Id: faceops.c 2.15 1997/04/01 11:27:20 johng Exp $
 * $Locker: $
 *
...
BR_RCS_ID("$Id: faceops.c 2.15 1997/04/01 11:27:20 johng Exp $");
	...
	struct v11face *fp, struct temp_face *tfp)
{
	...
	if((clipped = FaceClip(rend.renderer, clip_in, rend.renderer->state.cache.clip_slots, tfp->codes, 3, &nclipped)))
		ClippedRenderTriangles(rend.renderer, block->chain, clipped, nclipped, fp, tfp);
}
	...
	if((clipped = FaceClip(rend.renderer, clip_in, rend.renderer->state.cache.clip_slots, tfp->codes, 3, &nclipped))) {
		...
		if(tfp->flag & TFF_REVERSED) {
			...
			for(i=0; i < rend.renderer->state.cache.nconstant_fns; i++)
				rend.renderer->state.cache.constant_fns[i](rend.renderer, &vp->p, &vp->map, &rev_normal, colour, clipped->comp);
...
#endif
			for(i=0; i < rend.renderer->state.cache.nconstant_fns; i++)
				rend.renderer->state.cache.constant_fns[i](rend.renderer, &vp->p, &vp->map, (br_vector3 *)&fp->eqn, colour, clipped->comp);
		}
	...
	struct v11face *fp, struct temp_face *tfp)
{
	...
	for(i=0; i < rend.renderer->state.cache.nconstant_fns; i++)
		rend.renderer->state.cache.constant_fns[i](rend.renderer, &vp->p, &vp->map, (br_vector3 *)&fp->eqn, colour, v0->comp);
	...
	struct v11face *fp, struct temp_face *tfp)
{
	...
	if(tfp->flag & TFF_REVERSED) {
		...
		for(i=0; i < rend.renderer->state.cache.nconstant_fns; i++)
			rend.renderer->state.cache.constant_fns[i](rend.renderer, &vp->p, &vp->map, &rev_normal, colour, v0->comp);
	...
	if(tfp->flag & TFF_REVERSED) {
		...
		for(v=0; v<3; v++) {
			...
			if((tv[v].flags & (TVDIR_FRONT | TVDIR_BACK)) == (TVDIR_FRONT | TVDIR_BACK)) {
				...
				vp = rend.vertices+fp->vertices[v]; 
				...
				for(i=0; i < rend.renderer->state.cache.nvertex_fns; i++)
					rend.renderer->state.cache.vertex_fns[i](rend.renderer, &vp->p, &vp->map, &rev_normal, colour, tv[v].comp);
			}
		}
		block->chain->render(block->chain, tv+0, tv+1, tv+2, fp, tfp);
	} else {
		block->chain->render(block->chain, v0, v1, v2, fp, tfp);
	}
}
